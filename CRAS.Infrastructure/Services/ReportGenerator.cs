...
public class ReportGenerator : IReportGenerator
{
    public byte[] Generate(Contractor contractor, AggregatedRiskResult assessment)
    {
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(col => ComposeHeader(col, contractor, assessment));

                page.Content().Column(col =>
                {
                    col.Spacing(20);
                    ComposeRiskAssessment(col, assessment);
                    ComposeFinancialRatios(col, contractor.FinancialStatements.OrderByDescending(s => s.Year).FirstOrDefault());
                    ComposePaymentBehavior(col, contractor);
                    ComposeFinancialHistory(col, contractor);
                });
    ...
    private static void ComposeHeader(ColumnDescriptor col, Contractor contractor, AggregatedRiskResult assessment)
    {
        col.Item().Row(row =>
        {
            ...
            row.ConstantItem(150).AlignRight().Column(inner =>
            {
                ...
                inner.Item().Text(assessment.OverallRiskLevel.ToString().ToUpper()).FontSize(16).SemiBold().FontColor(riskColor).AlignRight();
            });
        });
    ...
    private static void ComposeRiskAssessment(ColumnDescriptor col, AggregatedRiskResult assessment)
    {
    ...
    private static void ComposeFinancialRatios(ColumnDescriptor col, FinancialStatement? latestStatement)
    {
    ...
    private static void ComposePaymentBehavior(ColumnDescriptor col, Contractor contractor)
    {
        col.Item().Text("Payment Behavior").FontSize(14).SemiBold().FontColor(Colors.Blue.Darken2);

        if (contractor.Invoices.Count == 0)
        {
            col.Item().Text("No payment history available.").FontColor(Colors.Grey.Medium).Italic();
            return;
        }
    ...
    private static void ComposeFinancialHistory(ColumnDescriptor col, Contractor contractor)
    {
        col.Item().Text("Financial Statement History").FontSize(14).SemiBold().FontColor(Colors.Blue.Darken2);

        if (contractor.FinancialStatements.Count == 0)
        {
            col.Item().Text("No financial history available.").FontColor(Colors.Grey.Medium).Italic();
            return;
        }
        ...
        col.Item().Table(table =>
        {
            ...
            table.Header(header =>
            {
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).Text("Year").SemiBold().FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("Assets").SemiBold()
                    .FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("Liabilities").SemiBold()
                    .FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("Working Cap").SemiBold()
                    .FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("EBIT").SemiBold()
                    .FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("Sales").SemiBold()
                    .FontSize(9);
                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingBottom(5).AlignRight().Text("Net Income").SemiBold()
                    .FontSize(9);
            });

            foreach (var statement in contractor.FinancialStatements.OrderByDescending(s => s.Year))
            {
                table.Cell().PaddingVertical(4).BorderBottom(1).BorderColor(Colors.Grey.Lighten3).Text(statement.Year.ToString()).FontSize(9);
                table.Cell().PaddingVertical(4).BorderBottom(1).BorderColor(Colors.Grey.Lighten3).AlignRight()
                    .Text(statement.TotalAssets.ToString("N0")).FontSize(9);
                table.Cell().PaddingVertical(4).BorderBottom(1).BorderColor(Colors.Grey.Lighten3).AlignRight()
                    .Text(statement.TotalLiabilities.ToString("N0")).FontSize(9);
